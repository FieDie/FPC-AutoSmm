...
PROMOCODES_FILE = f"storage/plugins/{UUID}/promocodes.json"
PROMOCLAIMS_FILE = f"storage/plugins/{UUID}/promo_claims.json"
...
def save_promocodes(data: dict) -> None:
    os.makedirs(f"storage/plugins/{UUID}", exist_ok=True)
    with open(PROMOCODES_FILE, "w") as file:
        json.dump(data, file, indent=4)


def load_promo_claims() -> dict:
    if os.path.exists(PROMOCLAIMS_FILE):
        with open(PROMOCLAIMS_FILE, "r") as file:
            return json.load(file)
    return {}


def save_promo_claims(data: dict) -> None:
    os.makedirs(f"storage/plugins/{UUID}", exist_ok=True)
    with open(PROMOCLAIMS_FILE, "w") as file:
        json.dump(data, file, indent=4)
...
def send_welcome_message(c: Cardinal, order: Dict) -> None:
    """
    Отправляет приветственное сообщение после оплаты заказа
    """
    welcome_text = f"""🎉 Спасибо за ваш заказ!

🔗 Теперь отправьте ссылку на ваш пост/канал/группу

📝 Примеры для ссылок: https://telegra.ph/Primery-ssylok-dlya-nakrutki-08-12

⚠️ Важно: ссылка должна вести именно на пост/видео/канал/профиль, который нужно накрутить.

Отправьте ссылку одним сообщением 👇"""
    
    try:
        c.send_message(order['chat_id'], welcome_text)
...
def order_handler(c: Cardinal, e: NewOrderEvent, id_value, quan_value, buyer_uz, type_api='API_1') -> None:
    try:
        ...
        # Определяем chat_id покупателя для дальнейшей коммуникации
        chat_id = ""
        try:
            chat_id = c.account.get_chat_by_name(buyer_uz).id
        except Exception as ex:
            logger.warning(f"Не удалось определить chat_id для покупателя {buyer_uz}: {ex}")
        
        current_datetime = datetime.now().strftime("%Y-%m-%d %H:%M:%S") #час бачив? пісяти і спати

        # применяем промокод, если активирован у покупателя (только на один заказ)
        try:
            claims = load_promo_claims()
            claim = claims.get(str(buyer_uz))
            if claim and not claim.get("used", False):
                percent = int(claim.get("percent", 10))
                bonus = int(orderAmount * percent / 100)
                if bonus > 0:
                    orderAmount += bonus
                    # отметим как использованный
                    claim["used"] = True
                    claim["used_at"] = current_datetime
                    claims[str(buyer_uz)] = claim
                    save_promo_claims(claims)
                    # уведомление покупателю
                    try:
                        if chat_id:
                            c.send_message(chat_id, f"🎁 Промокод применён: +{percent}% к заказу. Начислено дополнительно: {bonus}.")
                    except Exception:
                        logger.debug("Не удалось отправить уведомление о применении промокода", exc_info=True)
        except Exception:
            logger.debug("Ошибка при применении промокода", exc_info=True)
        ...
        # Отправляем приветственное сообщение после оплаты
        if chat_id:
            send_welcome_message(c, current_order_data)
        else:
            logger.warning("Приветственное сообщение не отправлено: chat_id не определён.")
...
def msg_hook(c: Cardinal, e: NewMessageEvent) -> None:
    ...
    # Команда покупателя: активация промокода
    try:
        if message_text.lower().startswith("/promocode"):
            parts = message_text.split()
            if len(parts) < 2:
                c.send_message(msg.chat_id, "Использование: /promocode CODE")
                return
            code = parts[1].strip()
            promocodes = load_promocodes()
            pdata = promocodes.get(code)
            if not pdata or (isinstance(pdata, dict) and not pdata.get("active", True)):
                c.send_message(msg.chat_id, "🔴 Промокод недействителен или истёк.")
                return
            claims = load_promo_claims()
            buyer_key = str(msgname)
            existing = claims.get(buyer_key)
            if existing and not existing.get("used", False):
                c.send_message(msg.chat_id, "ℹ️ У вас уже активирован промокод. Он применится к ближайшему заказу.")
                return
            percent = int(pdata.get("percent", 10)) if isinstance(pdata, dict) else 10
            claims[buyer_key] = {
                "code": code,
                "percent": percent,
                "used": False,
            ...
            save_promo_claims(claims)
            c.send_message(msg.chat_id, f"✅ Промокод принят. +{percent}% будет применено к следующему заказу.")
            return
    except Exception:
        logger.debug("Ошибка обработки команды /promocode", exc_info=True)
    ...
    elif len(command_parts) >= 2 and command_parts[0] == "#продавец":
        smm_id = command_parts[1]
        orders_map = load_orders()
        order_ref = None
        # Поиск по строковому и целочисленному ключу
        if smm_id in orders_map:
            order_ref = orders_map[smm_id]
        else:
            try:
                k_int = int(smm_id)
                if k_int in orders_map:
                    order_ref = orders_map[k_int]
            except Exception:
                pass
        if not order_ref:
            c.send_message(msg.chat_id, "🔴 SMM ID не найден. Проверьте номер заказа.")
            return
        # Уведомление продавцу (всем авторизованным пользователям TG ПУ)
        try:
            users = load_authorized_users()
            if users:
                notify_text = (
                    f"Покупатель {msgname} вызывает вас в заказ {smm_id}\n"
                    f"@chelovekwtf"
                )
                for user_id in users:
                    try:
                        c.telegram.bot.send_message(user_id, notify_text)
                    except Exception:
                        logger.debug("🔴 Не удалось отправить уведомление продавцу", exc_info=True)
        except Exception:
            logger.debug("🔴 Ошибка при уведомлении продавца", exc_info=True)

        # Ответ покупателю
        try:
            c.send_message(msg.chat_id, f"✅ Продавец вызван в заказ {smm_id}. Ожидайте ответа.")
        except Exception:
            logger.debug("Не удалось отправить подтверждение покупателю", exc_info=True)
    ...
    if links:
        ...
        if not settings.get("set_tg_private", False):
            if "t.me" in link and ("/c/" in link or "+" in link):
                c.send_message(order['chat_id'], """❌ Данный тип ссылки не поддерживается!

📝 Для Telegram каналов/групп:
• Ссылка должна вести на конкретный пост в публичном канале
• Пример: https://t.me/channelname/123
• Не поддерживаются приватные каналы и группы

🔄 Отправьте публичную ссылку на пост 👇""")
                return
        order['url'] = link
        link = link.replace("https://", "")
        confirmation_text = f"""✅ Ссылка получена!
...
🔄 Для изменения ссылки - отправьте новую ссылку

⏳ После подтверждения заказ будет отправлен в работу!"""
        ...
        if text.strip() == "+":
            ...
            if isinstance(smm_order_id, (int, str)) and str(smm_order_id).isdigit():
                ...
                c.send_message(order['chat_id'], 
                            f"""📊 Ваш заказ успешно создан и отправлен SMM сервису!
                            🆔 ID заказа: {smm_order_id}
                            
                            📋 Доступные команды:
                             ▐ 📗 Узнать статус заказа: #{status} {smm_order_id}
                             ▐ 📙 Рефилл (если доступно): #рефилл {smm_order_id}
                             ▐ 📢 Позвать продавца: #продавец {smm_order_id}
                            
                            ⚠️ Используйте команду #продавец только в случае проблем с заказом!

                            ⌛ Время выполнения заказа от нескольких минут до 48 часов. В редких случаях возможны задержки.
                            """)
            else:
...
def send_smm_start_info(c: Cardinal) -> None:
    ...
    text_start = (
            ...
            f"<i>ℹ️ Авто-накрутка by {CREDITS}</i>"
        )
...
def process_orders(c: Cardinal):
    while True:
        ...
        def send_completion_message(c: Cardinal, order_id: str):
            ...
            # Перед отправкой проверяем статус заказа на FunPay: если уже подтверждён, не отправляем уведомление
            try:
                fp_order = c.account.get_order(orders[order_id]["order_id"])  # FunPay order object
                if fp_order and fp_order.status == OrderStatuses.CLOSED:
                    logger.info(
                        f"Пропускаю уведомление: заказ {orders[order_id]['order_id']} уже подтверждён покупателем.")
                    return
            except Exception:
                # Если не удалось проверить статус — продолжаем как раньше (логи ниже)
                logger.debug("Не удалось проверить статус заказа на FunPay перед уведомлением.", exc_info=True)

            message_text = (
                f"✅ Заказ #{orders[order_id]['order_id']} выполнен!\n"
                f"Пожалуйста, перейдите по ссылке https://funpay.com/orders/{orders[order_id]['order_id']}/ и нажмите кнопку «Подтвердить выполнение заказа».\n"
            )
            try:
...
def init_commands(cardinal: Cardinal, *args):
    ...
    settings_smm_keyboard = InlineKeyboardMarkup(row_width=2)
    ...
    set_usersm_settings = InlineKeyboardButton("🛠 Переключатели", callback_data='set_usersm_settings')
    ...
    about_btn = InlineKeyboardButton("ℹ️ О боте", callback_data='about_autosmm')
    twiboost_btn = InlineKeyboardButton("Все что нужно вам тут - ***ТЫК***", url="https://vexboost.ru/ref3582897")
    settings_smm_keyboard.row(set_api, set_api_key)
    settings_smm_keyboard.row(set_api_2, set_api_key_2)
    settings_smm_keyboard.row(pay_orders, active_orders)
    settings_smm_keyboard.add(set_usersm_settings)
    settings_smm_keyboard.add(about_btn)
    settings_smm_keyboard.add(twiboost_btn)
        
    def update_alerts_keyboard():
        alerts_smm_keyboard = InlineKeyboardMarkup(row_width=1)
        if settings.get("set_alert_neworder", False):
            set_alert_neworder = InlineKeyboardButton("🔔 Увед. о созданном заказе", callback_data='set_alert_neworder')
        else:
            set_alert_neworder = InlineKeyboardButton("🔕 Увед. о созданном заказе", callback_data='set_alert_neworder')
        if settings.get("set_alert_errororder", False):
            set_alert_errororder = InlineKeyboardButton("🔔 Увед. об ошибке создания", callback_data='set_alert_errororder')
        else:
            set_alert_errororder = InlineKeyboardButton("🔕 Увед. об ошибке создания", callback_data='set_alert_errororder')
        if settings.get("set_alert_smmbalance_new", False):
            set_alert_smmbalance_new = InlineKeyboardButton("🔔 Баланс до создания", callback_data='set_alert_smmbalance_new')
        else:
            set_alert_smmbalance_new = InlineKeyboardButton("🔕 Баланс до создания", callback_data='set_alert_smmbalance_new')
        if settings.get("set_alert_smmbalance", False):
            set_alert_smmbalance = InlineKeyboardButton("🔔 Баланс после создания", callback_data='set_alert_smmbalance')
        else:
            set_alert_smmbalance = InlineKeyboardButton("🔕 Баланс после создания", callback_data='set_alert_smmbalance')
        if settings.get("set_refund_smm", False):
    ...
    def send_settings(m: types.Message):
        header = (
            ...
            f"👨‍💻 <b>Автор:</b> <i>{CREDITS}</i>\n\n"
            f"⚙️ <b>Управление AutoSmm</b>\n\n"
            f"API 1: <code>ID:</code>\nAPI 2: <code>ID2:</code>\n\n"
            f"💡 <i>Подсказки:</i>\n"
            f"• Статус заказа: <code>#статус &lt;id&gt;</code>\n"
            f"• Рефилл: <code>#рефилл &lt;id&gt;</code>"
        )
        bot.reply_to(m, header, reply_markup=settings_smm_keyboard, parse_mode='HTML')

    def send_about(call: telebot.types.CallbackQuery):
        about_text = (
            f"🤖 <b>{NAME}</b> <code>v{VERSION}</code>\n\n"
            f"📝 <b>Описание:</b> Плагин авто-накрутки через SMM API.\n"
            f"👨‍💻 <b>Автор:</b> <i>{CREDITS}</i>\n\n"
            f"📘 <b>Команды:</b>\n"
            f"• <code>#статус &lt;id&gt;</code> — статус заказа\n"
            f"• <code>#рефилл &lt;id&gt;</code> — запрос на рефилл\n"
        )
        back_button = InlineKeyboardButton("⬅️ Назад", callback_data='set_back_butt')
        kb = InlineKeyboardMarkup().add(back_button)
        bot.edit_message_text(about_text, call.message.chat.id, call.message.message_id, reply_markup=kb, parse_mode='HTML')
    ...
    tg.cbq_handler(send_about, lambda c: c.data == 'about_autosmm')
    tg.msg_handler(
        handle_text_input, 
    ...
    # создание промокодов из Telegram ПУ
    def create_promo(m: telebot.types.Message):
        try:
            parts = m.text.split()
            if len(parts) < 2:
                bot.send_message(m.chat.id, "Использование: /autosmm_createpromo CODE [PERCENT]")
                return
            code = parts[1].strip()
            percent = 10
            if len(parts) >= 3:
            ...
            data[code] = {"percent": percent, "active": True}
            save_promocodes(data)
            bot.send_message(m.chat.id, f"✅ Промокод <code>{code}</code> создан. Бонус: {percent}%.")
        except Exception:
            logger.debug("Ошибка при создании промокода", exc_info=True)

    tg.msg_handler(create_promo, commands=["autosmm_createpromo"])
    cardinal.add_telegram_commands(UUID, [
        ("autosmm", f"настройки {NAME}", True),
        ("check_balance", f"баланс {NAME}", True),
        ("autosmm_createpromo", "создать промокод", True)
    ])
